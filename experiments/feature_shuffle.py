...
default_args = AttrDict(
    ...
    "layer_type": "GCN",
    "num_relations": 1,
    "ratio": 0.0 # added a shuffle ratio feature
    }
    )


def shuffle_features_within_class(data, train_mask, val_mask, test_mask, ratio):
    x = data.x.clone()
    y = data.y
    num_nodes = data.num_nodes
    num_classes = y.max().item() + 1

    # Convert index lists to full boolean masks if needed
    def to_bool_mask(mask):
        if isinstance(mask, list) or mask.dtype != torch.bool:
            new_mask = torch.zeros(num_nodes, dtype=torch.bool, device=data.x.device)
            new_mask[mask] = True
            return new_mask
        return mask
    
    train_mask = to_bool_mask(train_mask)
    val_mask = to_bool_mask(val_mask)
    test_mask = to_bool_mask(test_mask)
    
    def shuffle_subset(mask):
        for cls in range(int(num_classes)):
            class_indices = ((y == cls) & mask).nonzero(as_tuple=False).view(-1)
            n = int(len(class_indices) * ratio)
            if n >= 2:
                selected = class_indices[torch.randperm(len(class_indices))[:n]]
                permuted = selected[torch.randperm(len(selected))]
                x[selected] = x[permuted]

    shuffle_subset(train_mask)
    shuffle_subset(val_mask)
    shuffle_subset(test_mask)

    return x

class Experiment_shuffle:
    def __init__(self, args=None, dataset=None, ratio=None, train_mask=None, validation_mask=None, test_mask=None):
    ...
    def run(self):
        ...
        if self.args.ratio > 0.0:
            batch.x = shuffle_features_within_class(
                data=batch,
                train_mask=self.train_mask,
                val_mask=self.validation_mask,
